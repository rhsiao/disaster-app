...
var disaster = SAGE2_App.extend({
    getData: function(){
    d3.json("uploads/apps/disaster/exampledata.json", function(collection){
...
init: function(data) {
    ...
    addCSS("uploads/apps/disaster/scripts/leaflet.css",            function(){
        ...
        //d3: initializes leaflet as layer initialization. 
        mySelf.map._initPathRoot();

        //this is an SVG element for leaflet's overlay pane which repositions when the map pans. There's no width or height because the dimensions are set dynamically because they change on zoom. picks up svg from map object
        mySelf.svg = d3.select(mySelf.map.getPanes().overlayPane).append('svg');
        //there is a g (Group) element such that the top left corner of SVG corresponds with leaflet's layer origin. 
        mySelf.g = mySelf.svg.append('g');

        //need to attach SVG into the this.element node. this is our container
        var box = "0,0,"+width+','+height;
        mySelf.svg = d3.select(mySelf.leafletDiv).append("svg")
            .attr("width", mySelf.leafletDiv.style.width)
            .attr("height", mySelf.leafletDiv.style.height)
            .attr("viewBox", box);
    ...
    dealWithData: function(data){
        ...
        mySelf.getData();
        var collection = data.collection;
        var feature = mySelf.g.selectAll("circle")
        .data(collection)
        .enter()
        .append("svg:circle")
        .attr("r", 100000)
